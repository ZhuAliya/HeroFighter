...
namespace Control
{
    public class Ctr_Warrior_AI : BaseControl {
        ...
        void Start()
        {
            ...
            /*确定个体差异性参数 */
            FloMoveSpeed = UnityHelper.GetInstance().GetRandomNum(3, 8);            //移动速度随机1-2
           // FloAttackDistance = UnityHelper.GetInstance().GetRandomNum(1, 2);       //攻击距离随机值1-3
            FloCordonDistance = UnityHelper.GetInstance().GetRandomNum(4, 15);   //警戒距离默认的随机值距离为4-15
            FloThinkInterval = UnityHelper.GetInstance().GetRandomNum(1, 3);         //思考的时间随机值1-3
        ...
        {
            //this.transform.rotation = Quaternion.Slerp(this.transform.rotation, Quaternion.LookRotation(new Vector3(_GoHero.transform.position.x, 0, _GoHero.transform.position.z) -
            //                                                                  new Vector3(_MyTransform.position.x, 0, _MyTransform.position.z)),    FloRotateSpeed          
            //                                                                 );
